...
export const CommandsTable = () => {
  ...
  const setPillStyle = (text: string) => {
    switch (text.toLocaleLowerCase()) {
      case 'everyone':
        return {
          backgroundColor: alpha('var(--mantine-color-primary)', 0.75),
          color: alpha('var(--mantine-color-white)', 0.75),
        }
      case 'moderator':
        return {
      ...
      case 'broadcaster':
        return {
          backgroundColor: 'rgba(205, 135, 135, 0.85)',
          color: '#FFE3E3',
        }
    }
  }
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          'http://localhost:4000/public/commands.json',
        )
        if (!response.ok) {
        ...
        const commands: Command[] = jsonData.filter((c: Command) =>
          isAuthenticated ? c : c.enabled,
        )
        setData(commands)
        setError(null)
      } catch (err) {
    ...
    <Paper withBorder>
              ...
              {isAuthenticated && (
                <Table.Td>
                  <Switch
                    ...
                    onChange={(_event) => toggleRow(item.id)}
                  />
                </Table.Td>
              )}
              <Table.Td miw={'12em'} p="sm">
                !{item.name}
              </Table.Td>
              <Table.Td miw={'12em'}>
              ...
              <Table.Td>
                {item.permissions ? (
                  <Pill
                    radius="sm"
                    fw="700"
                    style={setPillStyle(item.permissions[0])}
                  >
                    {item.permissions[0]}
                  </Pill>
                ) : (
                  <Pill radius="sm" fw="700" style={setPillStyle('Everyone')}>
                    Everyone
                  </Pill>
                )}
              </Table.Td>
      ...
      <Table stickyHeader width="100%" hiddenFrom="md">
        <Table.Thead bg={'transparent'}>
          <Table.Tr>
            <Table.Th colSpan={2}>Command</Table.Th>
          </Table.Tr>
        </Table.Thead>
        <Table.Tbody>
          {data.map((item) => (
