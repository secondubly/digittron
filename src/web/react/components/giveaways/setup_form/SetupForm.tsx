...
const BackButton: React.FC<{ disabled: boolean; back: () => void }> = ({
  disabled,
  back,
}) => {
  return (
    <Button disabled={disabled} onClick={back}>
      Back
    </Button>
  )
}
  ...
  return (
    <form onSubmit={form.onSubmit(console.log)} className="giveawayForm">
      <Stepper active={activePage} styles={{ steps: { display: 'none' } }}>
        ...
        <Stepper.Step>
          <SetupFormStepThree
            giveawayType={form.getValues().giveawayType}
            form={form}
          />
        </Stepper.Step>
      </Stepper>
