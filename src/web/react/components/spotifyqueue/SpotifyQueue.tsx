      ...
      <Flex gap={'10em'} visibleFrom="md">
        <Group className={classes['now-playing-container']}>
          <Image
            ...
            radius="lg"
            className={classes['now-playing-album']}
          />
          <Stack justify="center" h={'100%'} gap="xs">
            <Title
              order={4}
              className={classes['now-playing-title']}
              c={'gray'}
            >
              Now Playing
            </Title>
        ...
        <Box className={classes['queue-container']}>
          <Box
            ...
            style={{
              borderBottom:
                '1px solid var(--mantine-color-light-dark(dark-3,light.3))',
            }}
          >
            ...
            >
              <Box component="ol" className="queue-list">
                {queue.map((track: SimplifiedTrack, index: number) => (
                  <li
                    key={`${track.id}-${index}`}
                    className={classes['queue-item']}
                  >
                    <Image
                      src={track.album.images[0].url}
                      radius="md"
                      className={classes['queue-album-art']}
                    />
                    <Stack gap="0" className={classes['queue-item-track-info']}>
                      <Text lineClamp={1}>{track.name}</Text>
                      <Text lineClamp={1} c="dark.1">
                        {track.artists[0]?.name}
                      </Text>
                    </Stack>
                    <Text className={classes['queue-item-duration']}>
                ...
                >
                  ...
                  <Stack gap="0" className={classes['queue-item-track-info']}>
                    ...
                    <Text
                      lineClamp={1}
                      style={{
                        color: 'light-dark(#42443F, #CFD1CC)',
                      }}
                    >
                      {track.artists[0]?.name}
                    </Text>
                  ...
                  <Text
                    className={classes['queue-item-duration']}
                    style={{
                      color: 'light-dark(#42443F, #CFD1CC)',
                    }}
                  >
